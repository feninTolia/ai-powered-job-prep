...
import { AlertCircle, CheckCircle, UploadIcon, XCircle } from 'lucide-react';
...
const ResumePageClient = ({ jobInfoId }: Props) => {
  ...
  return (
    <div className="space-y-8 w-full">
      <Card>
        ...
        <CardContent>
          <LoadingSwap loadingIconClassName="" isLoading={isLoading}>
            <div
              role="cell"
              className={cn(
      ...
      <AnalysisResult aiAnalysis={aiAnalysis} isLoading={isLoading} />
    </div>
  );
};
                    ...
                    <div className="space-y-3">
                      {category?.feedback == null ? (
                        ...
                        category.feedback.map((item, idx) => {
                          ...
                          return <FeedbackItem key={idx} item={item} />;
                        })
                      )}
                    </div>
                  </div>
...
function FeedbackItem({
  ...
  item: Partial<z.infer<typeof aiAnalyzeSchema>['ats']['feedback'][number]>;
}) {
  const { message, type, name } = item;
